...
let adatok: { helyezes: number, sportolokSzama: number, sportag: string, versenyszam: string }[] = [];

for (let i = 0; i < helsinkiAdat.length; i++) {
    let sor: string = helsinkiAdat[i];
    let elemek: string[] = sor.split(" ");

    let helyezes: number = parseInt(elemek[0]);
    let sportolokSzama: number = parseInt(elemek[1]);
    let sportag: string = elemek[2];
    let versenyszam: string = elemek.slice(3).join(" ");
...
let pontszerzoHelyezesek: number = 0;

for (let i = 0; i < adatok.length; i++) {
    let helyezes: number = adatok[i].helyezes;
...
let osszesErme: number = ermek.arany + ermek.ezust + ermek.bronz;
...
for (let i: number = 0; i < adatok.length; i++) {
    let sportag: string = adatok[i].sportag;
    let helyezes: number = adatok[i].helyezes;
...
if (uszasErmek > tornaErmek) {
    console.log("Az uszás sportágban több érmet szereztek a sportolók.");
} else if (tornaErmek > uszasErmek) {
    console.log("A torna sportágban több érmet szereztek a sportolók.");
} else {
    console.log("Egyenlő volt az érmek száma a sportágak között.");
}
